...
/// @example cpu_sdpa_dummy.cpp
/// PoC: SDPA as a first-class CPU primitive (architecture demo).
///
...
void cpu_sdpa_dummy_example() {
    // --- engine + stream ---
    engine eng(engine::kind::cpu, 0);
    stream s(eng);

    // --- shapes: 4D (N, H, S, D) ---
    const memory::dim N = 2, H = 4, S = 16, D = 32;
    const memory::dims q_dims = {N, H, S, D};        // (N, H, Sq,  D)
    const memory::dims k_dims = {N, H, D, S};        // (N, H, D,  Skv) -- K is pre-transposed
    const memory::dims v_dims = {N, H, S, D};        // (N, H, Skv, D)
    const memory::dims o_dims = {N, H, S, D};        // (N, H, Sq,  D)

    // --- describe Q, K, V, dst ---
    memory::desc q_md(q_dims, memory::data_type::f32, memory::format_tag::abcd);
    memory::desc k_md(k_dims, memory::data_type::f32, memory::format_tag::abcd);
    ...
    std::vector<float> k_data(product(k_dims), 0.2f);
...
int main(int argc, char **argv) {
    return handle_example_errors({engine::kind::cpu}, cpu_sdpa_dummy_example);
}
