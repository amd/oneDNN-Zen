...
const std::map<pk_impl_key_t, std::vector<impl_list_item_t>> &impl_list_map() {
    // clang-format off
    static std::map<pk_impl_key_t, std::vector<impl_list_item_t>> the_map =  REG_EMBEDDING_BAG_P({
        {{forward}, {
            CPU_INSTANCE(ref_embedding_bag_fwd_t)
            nullptr,
        }},
    });
    // clang-format on
...
const impl_list_item_t *get_embedding_bag_impl_list(const embedding_bag_desc_t *desc) {
